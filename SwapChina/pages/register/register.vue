<template>
  <view class="register-container" :class="{'dark': theme === 'dark'}">
    <view class="register-content">
      ...
      <view class="header-section">
        ...
        <text class="register-title">{{ t.auth.register }}</text>
        <text class="register-subtitle">{{ t.auth.joinTraders }}</text>
      </view>
      ...
      <view class="form-section">
        ...
        <Button 
          ...
          :loading="isLoading"
          :disabled="isLoading"
          :text="isLoading ? t.auth.sendingCode : t.auth.startNow"
          @tap="handleRegister"
        />

        <!-- 登录链接 -->
        <view class="footer-link">
          <text class="footer-text">{{ t.auth.alreadyHaveAccount }}</text>
          <text class="link-text" @tap="goToLogin">{{ t.auth.loginNow }}</text>
        </view>
      </view>
    </view>
...
import { register, sendVerificationCode } from '@/api/index.js'
...
const handleRegister = async () => {
  if (mode.value === 'email') {
    ...
    if (formData.value.password.length < 8) {
      showToast(t.value.auth.atLeast8Chars, 'error')
      return
    }
    ...
    try {
      isLoading.value = true
      // 邮箱注册：使用邮箱+验证码+密码
      await register(formData.value.email, formData.value.password, '', formData.value.emailCode)
      showToast(t.value.auth.registerSuccess, 'success')
      setTimeout(() => {
        uni.switchTab({
    ...
    try {
      isLoading.value = true
      // 手机注册：使用手机号+验证码+密码
      await register(formData.value.phone, formData.value.password, '', formData.value.code)
      showToast(t.value.auth.registerSuccess, 'success')
      setTimeout(() => {
        uni.switchTab({
          url: '/pages/index/index'
